    ...
    useEffect(() => {
        const fetchMachines = async () => {
            try {
                ...
                const typeDataFetchRequests = machinesTypeId.map((categoryId) => fetch(`https://machine-maintenance.ddns.net/api/maintenance/type/${categoryId}/`).then((res) => res.json()));

                const brandDataFetchRequests = machinesBrandId.map((categoryId) => fetch(`https://machine-maintenance.ddns.net/api/maintenance/brand/${categoryId}/`).then((res) => res.json()));


                const lineDataFetchRequests = machinesLineId.map((categoryId) => fetch(`https://machine-maintenance.ddns.net/api/production/lines/${categoryId}/`).then((res) => res.json()));

                const supplierDataFetchRequests = machinesSupplierId.map((categoryId) => fetch(`https://machine-maintenance.ddns.net/api/maintenance/supplier/${categoryId}/`).then((res) => res.json()));
                ...
                <tbody>
                    {
                        ...
                        machinesData.length === 0 && <tr><td className="loader" colSpan={10}>Loading...</td></tr>
                    }
                </tbody>
            </table>
        </>
